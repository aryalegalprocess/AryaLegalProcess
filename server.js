...
app.use(cors({
  ...
  methods: ["GET", "POST", "PUT", "DELETE"], // ✅ Added DELETE method
  credentials: true
}));
...
// ✅ Serve images with correct CORS headers
...
const CompanyModel = require('./models/company')(companyConnection); // ✅ Correct usage

// Default DB for Contact form
...
// Products
const productRoutes = require('./server/routes/products')(Product, CompanyModel);
app.use('/api/products', productRoutes);

// Companies
const companyRoutes = require('./server/routes/companies')(CompanyModel); // ✅ Use correct model
...
app.post('/api/send-expiry-emails', async (req, res) => {
  try {
    ...
    for (const companyId of companyIds) {
      const company = await CompanyModel.findOne({ id: parseInt(companyId) }); // ✅ Correct usage
...
app.get('/api/products/:barcode', async (req, res) => {
  try {
    ...
    const company = await CompanyModel.findOne({ id: parseInt(product.company) });
...
// Wait for DB connections before starting server
Promise.all([
